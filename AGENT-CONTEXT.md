...
- **527 pass.** NEXT Wave-2 (ranked): M (margin/liquidation guard — `liquidationPx` CAN BE NULL when far from liq; `crossMaintenanceMarginUsed`), N (funding signal), G (reconciliation halt), O/P (REPL header + CRUD), Q (rate-limit/WS).
