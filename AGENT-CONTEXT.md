...
- **530 pass.** L+K+M all COMMITTED + drive-verified. NEXT Wave-2 (ranked): N (funding signal — needs cross-hour funding-sign verification, shared with K-funding follow-on), G (reconciliation halt), O/P (REPL header + CRUD), Q (rate-limit/WS). Follow-ons: K sentry/abort fee-netting + real per-fill fees; M pre-fire margin gate.
