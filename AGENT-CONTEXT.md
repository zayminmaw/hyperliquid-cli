...
- **542 pass.** DONE + committed: L, K, M, O, P, G. NEXT Wave-2: Q (rate-limit/WS — lowest, verify current HL limits first). BLOCKED: N + K-funding (funding SIGN needs a position held across a funding hour — do not guess). Follow-ons: K sentry/abort fee-netting + real per-fill fees; M pre-fire maintenance-margin gate (`crossMaintenanceMarginUsed` verified); G auto-run at run_once/agent start.
