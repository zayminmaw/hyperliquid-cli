...
- **534 pass.** DONE + committed: L, K, M, O, P. NEXT Wave-2: G (reconciliation halt — mainnet hardening), then Q (rate-limit/WS). BLOCKED: N + K-funding (funding SIGN needs a position held across a funding hour — do not guess). Follow-ons: K sentry/abort fee-netting + real per-fill fees; M pre-fire maintenance-margin gate (`crossMaintenanceMarginUsed` — field verified).
