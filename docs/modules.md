...
| File | What it does | Key surface |
|------|--------------|-------------|
| `base.py` | The `Exchange` Protocol both backends satisfy. | `Exchange`, methods: `get_marks/get_book/get_candles/equity/get_positions/get_open_orders/recent_fills/place_order/cancel/cancel_all/set_leverage` (`recent_fills` = user-fills feed for real exit-price/fee reads; `[]` on paper) |
| `marks.py` | Public `/info` reads over **httpx** (keyless), TTL-cached (cache returned by copy); bounded jittered retry on 429/5xx/transport drops (honors `Retry-After`, clamped; fails fast on other 4xx; re-raises after `max_retries`); includes `candleSnapshot` for regime and `meta` for per-asset `szDecimals`. | `MarksFeed`, `MarksFeed.candles()`, `MarksFeed.sz_decimals()`, `api_url()` |
| `paper.py` | Simulated book on public marks; state-backed (persists in `state-paper.db`). | `PaperExchange` |
| `hyperliquid.py` | Live testnet/mainnet. Reads keyless (`frontendOpenOrders`, so trigger orders are visible to `cancel-all` and cleanup); writes need the agent key (SDK + `eth_account` lazy-imported) and are rounded to the asset's size/price precision on the wire (`rounding.py`). **`equity()` is unified-account aware:** it detects `userAbstraction=="unifiedAccount"` (cached) and, for unified accounts, reads the collateral from the spot clearinghouse (USDC) marked to market by open-position uPnL — the perp `accountValue` is only committed margin there, ~0 when flat. Standard accounts keep the `marginSummary.accountValue` read. | `HyperliquidExchange` (key only in `_agent_key`) |
...
| `gate.py` | The deterministic risk gate (first-failure wins, incl. mark sanity — mark present, inside sl/tp, R:R at mark ≥ floor; daily new-entry cap, audit B; account-wide gross exposure/leverage, audit A) + fixed-fractional sizing **at the mark** + side inference. The account-wide exposure check is shared with Mode A `trade` via `gross_exposure_reason()`/`book_gross_notional()`. | `evaluate()`, `GateContext` (`mark=`, `gross_notional=`, `trades_today=`), `GateOutcome`, `gross_exposure_reason()`, `book_gross_notional()`, `infer_side()` |
...
| `resolve.py` | The monitor step: close open trades on SL/TP/expiry → the `trades` ledger (won/lost/expired/closed, realized, R-multiple — R against `initial_sl`, and a stop-out on the profit side of entry books `won`, since sentry may have ratcheted the stop past entry). Realized P&L and R are booked **net of the round-trip taker fee** (`taker_fee`, wave-2 K), with the fee also stored in `fee_paid`; paper models the fee so paper↔live expectancy stays comparable. On live networks it also reconciles **vanished** positions (native trigger fired on a wick, or a manual close — outcome inferred from candle extremes, and priced at the **actual closing fill** via `recent_fills` when available, else the mark — item L), cancels surviving triggers after a close, and resolves shadow trades orderlessly. | `resolve_open_trades(…, shadow_only=)`, `_real_exit_price` |
| `monitor.py` | Read-only position-health view, incl. `liq_dist%` (distance to the exchange liquidation price, None when there's none — wave-2 M). | `position_health()` |
| `reconcile.py` | Order+position diff → safe / requires-halt verdict (wave-2 G): unexpected position, size mismatch, or an unprotected live position (a vanished ledger row is the resolver's job, not a halt). `hl exec reconcile` runs it and trips the breaker on divergence. | `reconcile()`, `ReconcileReport`, `Divergence` |
| `runner.py` | `run_once()` — the full pass orchestrator (resolve → re-check due WAIT deferrals → pull → enrich(+candles/regime/outcomes) → decide (skipped when the coin has no mark — the gate would reject anyway, so the paid call isn't spent) → defer-if-WAIT / gate → fire → open ledger row (**before** protection, so a crash never leaves an untracked position; abort resolves it `aborted`) → protect → log → advance HWM). An `act+wait` decision is parked in the `deferred` table and re-checked with fresh data (within freshness, up to `HL_FOLLOWUP_MAX_ATTEMPTS`, labeled `followup`); re-checks freeze while the breaker is tripped. Shadow books hypothetical trades; unmanaged exchange positions **and** positions within `HL_MIN_LIQUIDATION_DISTANCE_PCT` of their liquidation price (wave-2 M) raise edge-triggered critical alerts. Honors `dry_run` (fully side-effect-free), `fire_enabled` (shadow), injected `decide_fn`, and an `Alerter`. | `run_once()`, `PassSummary` (`seen/rechecked/approved/fired/rejected/failed/dropped/deferred/resolved/managed`) — `managed` = sentry 6a actions, run just before resolve |
...
`StateStore(path, read_only=True)` opens an existing book with no schema-create and no
migration (writes are refused) — used by `exec report --compare` to inspect another data
dir's book without touching it.
...
| `graduation.py` | Mainnet-readiness verdict: N resolved trades / N days / positive expectancy vs the hard caps. Surfaced in `exec report`. `graded_trades(trades)` is the one shared grading filter (partials/aborts/adopted excluded), reused by the `exec report --compare` A/B so the two can't drift. | `assess(trades, caps)`, `graded_trades(trades)` |
...
| `liveness.py` | Runner-liveness classification (audit F): is the supervisor's `LAST_TICK` heartbeat fresh? Pure, fail-closed (no/old heartbeat = not alive). Lets a **separate** reader (`agent status`, `agent watchdog`) detect a hard-killed loop that emits nothing. | `Liveness` (never/alive/stale), `classify()`, `stale_after_seconds()` |

Wired by `hl agent run|status|watchdog` (`cli/commands/agent.py`); `watchdog` is a
cron liveness reaper that pages a dead loop holding open positions. Cadences live on
the tunable surface (`TunableConfig.agent`, clamped); deploy templates in `deploy/`.
